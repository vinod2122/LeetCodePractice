...
1 <= maxSum <= 109
 */
public class Solution06_Dec_24 {
    public int maxCount(int[] banned, int n, int maxSum) {
        // Use a HashSet to store banned integers for fast lookup
        Set<Integer> bannedSet = new HashSet<>();
        for (int num : banned) {
            bannedSet.add(num);
        }

        int currentSum = 0; // Track the current sum of chosen integers
        int count = 0;      // Track the number of chosen integers
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (currentSum + i > maxSum) {
                break; // Stop as we can't add more numbers
            }
